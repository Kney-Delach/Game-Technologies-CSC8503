...
def save_msh(settings):
  ...
  for mesh in allMeshes:
    ...
    for v in mesh.data.vertices:
      ...
      allNormals.append(vNormal())
      allColours.append(vColour())

    for poly in mesh.data.polygons: 
      for vert in poly.vertices:
        allNormals[indexOffset + vert].AddValues(poly.normal)
  ...
  for mesh in allMeshes:
    ...
    for poly in mesh.data.polygons: 
      for loop_index in poly.loop_indices:
        ...
        if(len(mesh.data.vertex_colors) > 0):
          col_layer = mesh.data.vertex_colors[0].data
          allColours[startVertex + vIndex].SetValues(col_layer[loop_index].color[0], col_layer[loop_index].color[1],col_layer[loop_index].color[2],1.0)
